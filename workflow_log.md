...
- 工作根目錄：本機作業 workspace
- 作業需求檔案：workspace 中的 `Requirements.txt`
- 專案根目錄：`HW7`
- GitHub Repository：公開 GitHub repository

本機環境：

- Conda：本機 conda / miniforge
...
```text
可以幫我完成 HW7 的作業嗎？請以 HW7 作為專案根目錄，做一個 LLM + Diffusion 結合的生成式 AI 專題，題目可以叫做「AI Character Design Studio」。
...
- 顯示卡是 RTX 4080 16GB
- GitHub repo 是公開 GitHub repository
...
另外 GitHub 上只需要放程式碼和說明文件，不需要把 outputs 的生成結果或本機展示圖片推上去。
...
```text
可以將專案的說明等等都換成繁體中文嗎，並補上公開 GitHub repository 的資訊。
```
...
```text
可以幫我更新一下所有的文件敘述成最新的狀態嗎，包含workflow，README等等的，把我們觀察到的等等都寫進去這樣，然後我覺得推上去github的不需要我們的output，展示圖片也不用，只需要程式跟說明應該就可以了，然後關鍵prompt可以把我叫你設定的環境、專案內容的問答等等的，跟目錄指定等等也寫進去
```
